...
#         cuda: bool = False,
#     ) -> Union[T, KnnOutput]:
#         def _unit_pnorm(_input: T) -> T:
#             if isinstance(_input, Tensor):
#                 _output = F.normalize(_input, dim=1, p=self.p)
#             else:
#                 _output = _input / np.linalg.norm(x=_input, ord=self.p, axis=1, keepdims=1)
#             return _output  # type: ignore

#         x_np = x.detach().cpu().numpy() if isinstance(x, Tensor) else x
#         if self.normalize:
#             x = _unit_pnorm(_input=x)
...
#         else:
#             if self.normalize:
#                 x = _unit_pnorm(_input=y)
#             y_np = y.detach().cpu().numpy() if isinstance(y, Tensor) else y

#         index = self._build_index(d=x.shape[1])
#         if cuda or (isinstance(x, Tensor) and x.is_cuda) or (isinstance(y, Tensor) and y.is_cuda):
...
#         distances, indices = index.search(x=y_np, k=self.k)  # type: ignore
#         if isinstance(x, Tensor):
#             # Convert back from numpy to torch
#             indices = torch.as_tensor(indices, device=x.device)

#         if return_distances:
#             if (isinstance(x, Tensor) and x.requires_grad) or (
#                 isinstance(y, Tensor) and y.requires_grad
#             ):
